...
class GetViewDelegate internal constructor(provider: GetViewProvider) : GetDelegate<GetViewProvider>(provider) {
    ...
    private fun initToolbar() {
        ...
        with(toolbar) {
            // 初始化标题
            with(Get.init.toolbarInit) {
                // 原始界面默认不显示标题
                visibility = if (setGetContentViewRef.get() && visible) View.VISIBLE else View.GONE
                setToolbarInit(this)
//                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
//                    toolbarLayout.elevation = Sizes.dp2px(elevation).toFloat()
//                }
            }
